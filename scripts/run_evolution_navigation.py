...
def _write_report(args, cfg: NavConfig, p: dict) -> None:
    ...
    lines.append(
        ...
        "- **Contact-solver cap (performance).** A fly pinned against a physical "
        "obstacle generates a large contact set; MuJoCo's default Newton solver "
        "then does a dense Cholesky per iteration (up to 100), which blows a single "
        "rollout from ~2 s to ~40 s and stalls the run. Obstacle envs cap Newton at "
        "20 iterations (ls 10), bounding the worst-case rollout to ~7 s with the "
        "warm-start cue signs and collision headroom unchanged. Applied ONLY when "
        "obstacles are present, so chemo/loom/closed-loop/v1 physics stay byte-exact.\n"
